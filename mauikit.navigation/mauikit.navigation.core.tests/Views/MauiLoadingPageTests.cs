...
public class MauiLoadingPageTests : BaseViewTest
{
    [Fact]
    public void Should_Have_White_As_Default_LoaderColor()
    {
        var page = new MauiLoadingPage();
        page.LoaderColor.Should().Be(Colors.White);
    }
    ...
    {
        // Arrange
        var vm = new MauiViewModel(Substitute.For<INavigationService>())
        {
            IsBusy = false
        };

        var page = new MauiLoadingPage { BindingContext = vm };

        // Simulate MAUI template creation and attach it to the page
        var content = (Layout)page.ControlTemplate.CreateContent();
        content.BindingContext = vm;
        content.Parent = page;
        await Task.Yield();

        var overlay = content.FindByName<Grid>("LoadingOverlay");
